# Open WebUI Setup (Use StarAgent Instead of Ollama Directly)

StarAgent exposes an OpenAI-compatible API. Open WebUI can point to StarAgent as its “OpenAI” backend, and StarAgent will call Ollama locally.

## Preconditions

- Ollama reachable at `http://127.0.0.1:11434`
- StarAgent running (recommended port `8095`)
- Model installed in Ollama: `gemma4:e2b`

## What to Configure in Open WebUI

You want Open WebUI to call StarAgent’s OpenAI-compatible API:
...
curl -sS http://127.0.0.1:8095/v1/chat/completions \\
  ...
  -d '{\"model\":\"gemma4:e2b\",\"stream\":false,\"messages\":[{\"role\":\"user\",\"content\":\"Reply with exactly STARAGENT_OK\"}]}' \\
  | python3 -c 'import sys,json; print(json.load(sys.stdin)[\"choices\"][0][\"message\"][\"content\"])'
```

## Recommended Operator Defaults

- Start StarAgent with `HOST=0.0.0.0` so Docker containers can reach it.
- Keep StarAgent on `PORT=8095` to match recovered runtime validation.
